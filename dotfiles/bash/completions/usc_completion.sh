...
__usc_handle_go_custom_completion()
{
    ...
    if [ $((directive & shellCompDirectiveFilterFileExt)) -ne 0 ]; then
        ...
        __usc_debug "File filtering command: $filteringCmd"
        $filteringCmd
        ...
        if [ -n "$subdir" ]; then
            __usc_debug "Listing directories in $subdir"
            __usc_handle_subdirs_in_dir_flag "$subdir"
        else
            __usc_debug "Listing directories in ."
            _filedir -d
        fi
    else
